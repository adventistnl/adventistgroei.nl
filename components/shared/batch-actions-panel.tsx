...
export function BatchActionsPanel({
  ...
  useEffect(() => {
    const checkScreenSize = () => {
      ...
      setIsMobileView(forceMobileLayout || width < 768) // sm breakpoint
      setIsTabletView(width >= 768 && width < 1024) // md breakpoint
    }
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isMobileView ? "left-0 right-0 w-full" : "left-0 right-0",
        className
      )}
      style={!isMobileView ? {
    ...
    >
      <div className={cn(
        "px-3 sm:px-4 md:px-6 pb-3 sm:pb-4 md:pb-6",
        isMobileView ? "px-2 pb-2" : ""
      )}>
        <Card 
          ref={containerRef}
          className={cn(
            "border shadow-lg bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800",
            isMobileView ? "w-full mx-0" : "w-[90%] mx-auto",
            isTabletView ? "w-[95%]" : ""
          )}
        >
          ...
          {isMobileView ? (
            <div className="px-3 py-4 space-y-3">
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button
                        variant="outline"
                        size="sm"
                        className="w-full h-9 gap-2 text-sm text-gray-700 hover:text-gray-900 hover:bg-gray-100 dark:text-gray-300 dark:hover:text-gray-100 dark:hover:bg-gray-800"
                      >
                        <MoreHorizontal className="h-4 w-4" />
                        <span className="truncate">
                          {getTranslation('moreActions', 'More actions')} ({actions.length})
                        </span>
                      </Button>
                    </DropdownMenuTrigger>
              ...
              {summary && (
                <div className="border-t pt-3">
                  <div className="text-xs text-gray-500 dark:text-gray-500 opacity-80">
