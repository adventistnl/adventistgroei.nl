...
export interface EditFundingPolicyGroupModalProps {
  isOpen: boolean
...
export function EditFundingPolicyGroupModal({
  isOpen,
  onOpenChange,
  group,
  validations = [],
  onSuccess
}: EditFundingPolicyGroupModalProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedGroup: FundingPolicyGroup = {
        ...group,
        name: formData.name,
      ...
      // Here you would also update/create/delete validations
      // formData.validations.forEach(validation => {
      //   if (validation.tempId.startsWith('temp_')) {
      //     createValidation({ ...validation, group_id: group.id })
      //   } else {
      ...
      toast.dismiss(loadingToast)
      toast.error('Failed to update funding policy group')
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <DialogContent className="w-[95vw] max-w-3xl max-h-[95vh] overflow-hidden flex flex-col">
        <DialogHeader className="flex-shrink-0 pb-4">
          <DialogTitle className="flex items-center gap-2 text-lg">
            ...
            Edit Funding Policy Group
          </DialogTitle>
          <DialogDescription className="text-sm text-muted-foreground">
            Update the information and validation fields for this funding policy group
          </DialogDescription>
