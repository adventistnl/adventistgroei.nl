...
function ActivityForm({ formData, errors, onChange, translations, language }: ActivityFormProps) {
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="flex items-center justify-between p-3 sm:p-4 rounded-lg border bg-muted/50">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <div className={cn(
            "flex items-center justify-center w-8 h-8 rounded-full transition-colors flex-shrink-0",
            formData.request_subsidy ? "bg-primary/10" : "bg-muted"
          )}>
            <DollarSign className={cn(
              "w-4 h-4 transition-colors",
              formData.request_subsidy ? "text-primary" : "text-muted-foreground"
            )} />
          </div>
          <div className="flex-1 min-w-0">
            <Label 
              htmlFor="request_subsidy" 
              className="cursor-pointer font-medium text-sm leading-tight"
            >
              {translations.isSubsidizedActivity}
            </Label>
            <p className="text-xs text-muted-foreground mt-0.5">
              {translations.subsidyDescription}
            </p>
          </div>
        </div>
        <Switch
          id="request_subsidy"
          checked={formData.request_subsidy}
          onCheckedChange={(checked) => onChange({ request_subsidy: checked })}
          className="flex-shrink-0"
        />
      </div>

      {/* Institution Requested Amount - Only show if subsidized */}
      {formData.request_subsidy && (
        <FormField 
          label={translations.institutionRequestedAmount} 
          required 
          error={errors.institution_requested_amount} 
          htmlFor="institution_requested_amount"
        >
          <div className="space-y-2">
            <Input
              id="institution_requested_amount"
              type="number"
              placeholder={translations.institutionAmountPlaceholder}
            ...
            {formData.budget_amount > 0 && ((formData.institution_requested_amount ?? 0) > 0) && (
              <div className="flex items-center justify-between text-xs text-muted-foreground px-2">
                <span>
                  {Math.round(((formData.institution_requested_amount ?? 0) / formData.budget_amount) * 100)}% do orçamento
                </span>
                <span>
                  Igreja: € {(formData.budget_amount - (formData.institution_requested_amount ?? 0)).toLocaleString()}
                </span>
              </div>
            )}
          </div>
