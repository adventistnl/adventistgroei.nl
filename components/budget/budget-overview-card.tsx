  ...
  return (
    <Card data-chart={id} className={`flex flex-col ${className}`}>
      ...
      {!budgetKpisLoading && institutionBudgetData.totalBudget > 0 && (
        <div className="mt-4 pt-4 border-t mx-6 pb-6">
          ...
          <div className="grid grid-cols-3 gap-4 text-center mb-4">
            ...
            <div>
              ...
              <p className="font-bold text-lg" style={{ color: 'var(--chart-2-green)' }}>
                {Math.round((institutionBudgetData.budgetRemaining / institutionBudgetData.totalBudget) * 100)}%
              </p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">
                {currentLanguage === 'pt' ? 'Departamentos' : currentLanguage === 'nl' ? 'Afdelingen' : 'Departments'}
              </p>
          ...
          {/* Distribuição simplificada - apenas os 3 principais departamentos */}
          {processedDepartments.length > 0 && (
            <div className="space-y-2">
              <p className="text-muted-foreground text-xs font-medium text-center">
                {currentLanguage === 'pt' ? 'Principais Alocações' : currentLanguage === 'nl' ? 'Hoofdtoewijzingen' : 'Top Allocations'}
              </p>
              <div className="space-y-1">
                {processedDepartments.slice(0, 3).map((dept, index) => {
                  const color = getDepartmentColor(index)
                  return (
                    <div key={dept.id} className="flex items-center justify-between text-xs">
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <span className="text-muted-foreground truncate text-xs">{dept.departmentName}</span>
                      </div>
                      <span className="font-semibold text-xs" style={{ color }}>{dept.percentageOfTotal}%</span>
                    </div>
                  )
                })}
                {processedDepartments.length > 3 && (
                  <div className="flex items-center justify-center pt-1">
                    <span className="text-xs text-muted-foreground">
                      +{processedDepartments.length - 3} {currentLanguage === 'pt' ? 'outros' : currentLanguage === 'nl' ? 'anderen' : 'others'}
                    </span>
                  </div>
                )}
              </div>
