...
import { createSubsidyKanbanMoveRules } from "@/lib/subsidy-kanban-rules"
...
// ---------------------------------------------------------------------------
// Status-transition action definitions
// Each entry describes one possible move from any status → toStatus.
// `fromStatuses` (optional) restricts the source; otherwise canChangeStatus
// is the sole gatekeeper.
// ---------------------------------------------------------------------------
interface StatusTransitionActionDef {
  id: string
  toStatus: string
  fromStatuses?: string[]
  labelKey: string // key in translations.actions
  icon: React.ComponentType<{ className?: string }>
  variant?: 'destructive'
  separator?: boolean
}

const STATUS_TRANSITION_ACTIONS: StatusTransitionActionDef[] = [
  { id: 'mark_in_review',           toStatus: 'in_review',         fromStatuses: ['pending'],                     labelKey: 'markInReview',          icon: Eye },
  { id: 'send_back_pending',        toStatus: 'pending',           fromStatuses: ['in_review'],                   labelKey: 'sendBackPending',       icon: RotateCcw },
  { id: 'approve',                  toStatus: 'approved',          fromStatuses: ['pending', 'in_review'],        labelKey: 'approve',               icon: CheckCircle,  separator: true },
  { id: 'reject',                   toStatus: 'rejected',          fromStatuses: ['pending', 'in_review'],        labelKey: 'reject',                icon: XCircle,      variant: 'destructive' },
  { id: 'reopen',                   toStatus: 'approved',          fromStatuses: [''],                            labelKey: 'reopen',                icon: RotateCcw },
  { id: 'close',                    toStatus: 'closed',                                                           labelKey: 'close',                 icon: X,            separator: true },
  { id: 'mark_advanced_closed',     toStatus: 'advanced_closed',                                                  labelKey: 'markAdvancedClosed',    icon: CheckCircle2 },
  { id: 'mark_waiting_documents',   toStatus: 'waiting_documents',                                                labelKey: 'markWaitingDocuments',  icon: FileText },
  { id: 'mark_waiting_refund',      toStatus: 'waiting_refund',    fromStatuses: ['approved','waiting_documents'],           labelKey: 'markWaitingRefund',     icon: DollarSign },
]
...
export function SubsidyRequestManager({
  ...
  // Only Finance users can manage after approved
  const canManagePostApproved = (subsidy: SubsidyRequest | any): boolean => {
  ...
  // Returns the status-change actions the current user is actually allowed
  // to perform for the given subsidy, based on canChangeStatus rules.
  const getAvailableStatusActions = (subsidy: SubsidyRequest | any) => {
    const hasPending = (subsidy.receipts ?? []).some((r: any) => !r.is_validated && !r.is_deleted)
    const hasRejected = (subsidy.receipts ?? []).some((r: any) => r.is_validated && !r.approved && !r.is_deleted)

    const statusCtx: SubsidyStatusContext = {
      is_for_advance: subsidy.is_for_advance ?? false,
    ...
    const roleCtx: UserRoleContext = {
      isFinanceUser,
      isOwnerOrLeader: canApproveSubsidy(subsidy),
    }

    const currentStatus = subsidy.status ?? ''

    return STATUS_TRANSITION_ACTIONS.filter(action => {
      if (action.toStatus === currentStatus) return false
      if (action.fromStatuses && !action.fromStatuses.includes(currentStatus)) return false
      return canChangeStatus(currentStatus, action.toStatus, statusCtx, roleCtx)
    }).map(action => ({
      ...action,
      label: (translations.actions as unknown as Record<string, string>)[action.labelKey] ?? action.labelKey,
      onClick: () => handleKanbanItemMove(subsidy.id, currentStatus, action.toStatus),
    }))
  }
  ...
  const getResponsibleUsers = (subsidy: any): UserAvatarData[] => {
      ...
      if (leader) {
        ...
        if (!isDuplicate) {
          users.push({
            ...
            isCoOwner: true // Department Leader has elevated permissions (blue styling)
          })
        }
      }
    }
    ...
    if (collaborators && Array.isArray(collaborators)) {
      collaborators.forEach((collab: any) => {
        const collaboratorUser = collab.user
        if (collaboratorUser) {
          const isDuplicate = users.some(u => u.id === collaboratorUser.id)
          if (!isDuplicate) {
            users.push({
              id: collaboratorUser.id,
              name: collaboratorUser.name,
              email: collaboratorUser.email || undefined,
              role: collab.role || t('subsidy.roles.collaborator') || 'Collaborator',
              isOwner: false
            })
          }
        }
    ...
    financeUsers.forEach((fu: any) => {
      ...
      if (!isDuplicate) {
        users.push({
          ...
          isFinance: true // Finance users have special permissions (orange styling)
        })
      }
    })
  ...
  const subsidyColumns: ColumnDef<SubsidyRequest>[] = [
        ...
        return (
          <div className="flex justify-center">
            <UsersAvatarGroup
              ...
              coOwnerUserId={users.find(u => u.isCoOwner)?.id}
            />
          </div>
        )
      },
    },
    {
      ...
      cell: ({ row }) => {
        const availableActions = getAvailableStatusActions(row.original)
        return (
  ...
  // Kanban groups derived from central status registry (auto-updates when new statuses are added)
  const { kanbanGroups } = useSubsidyStatuses()
  ...
  const kanbanItems: KanbanItem[] = filteredSubsidyRequests.map(request => ({
    ...
    metadata: {
      ...
      // Store subsidy data for permission checks in moveRules
      subsidyData: request
    }
  }))

  // Kanban move rules — delegated to the reusable factory in lib/subsidy-kanban-rules.ts
  const kanbanMoveRules = useMemo(
    ...
    [isFinanceUser, canApproveSubsidy, canManagePostApproved, canChangeStatus, getStatusError]
  )
  ...
  const renderKanbanItem = (item: KanbanItem, group: KanbanGroup, dragHandlers?: any) => {
    const subsidyForItem = item.metadata?.subsidyData as SubsidyRequest
    const availableItemActions = subsidyForItem ? getAvailableStatusActions(subsidyForItem) : []
      ...
      >
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem
                onClick={(e) => { e.stopPropagation(); handleViewSubsidy(item.id) }}
              >
                <Settings className="mr-2 h-4 w-4" />
                {translations.actions.manageSubsidy}
              </DropdownMenuItem>
              {availableItemActions.length > 0 && (
                <>
                  ...
                  {availableItemActions.map(action => {
                    ...
                    return (
                      <React.Fragment key={action.id}>
                        {action.separator && availableItemActions.indexOf(action) > 0 && (
                          <DropdownMenuSeparator />
                        )}
                        <DropdownMenuItem
