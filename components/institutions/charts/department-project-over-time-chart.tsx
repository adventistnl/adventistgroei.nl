...
export function DepartmentProjectOverTimeChart({ 
  ...
  if (churchDepartments.length === 0 || filteredData.length === 0) {
    return (
      <Card className="h-full flex flex-col">
        <CardHeader className="border-b py-5">
          <CardTitle>{t.charts?.activity_overview?.title || "Projects Created Over Time"}</CardTitle>
          <CardDescription>
            {t.charts?.activity_overview?.description || "Project creation timeline by church departments"}
          </CardDescription>
        </CardHeader>
    ...
    <Card className="h-full flex flex-col">
      <CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
        <div className="grid flex-1 gap-1">
          <CardTitle>{t.charts?.activity_overview?.title || "Projects Created Over Time"}</CardTitle>
          <CardDescription>
            {t.charts?.activity_overview?.description || `Project creation timeline by church departments - ${getTimeRangeLabel(timeRange)}`}
          </CardDescription>
        </div>
